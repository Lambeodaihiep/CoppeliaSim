...
if (clientID>-1)
    ...
    while 1
        ...
        if command == 1
            ...
            promptForward = {'Enter q1 (degree)','Enter q2 (degree)', 'Enter q3 (degree)', 'Enter q4 (degree)', 'Enter q5 (degree)','Enter q6 (degree)'};
            ...
            [y, x, z] = UR10_forwardKinematic(q(1), q(2), q(3), q(4), q(5), q(6));
            disp("xE = " + num2str(x));
            disp("yE = " + num2str(y));
            disp("zE = " + num2str(z));
        elseif command == 2
            ...
            if trajectoryCommand == 1
                ...
                circlePrompt = {'Enter height', 'Enter center X', 'Enter center y'};
                circleCommand = inputdlg(circlePrompt, circleDlgtitle);
                %radius = str2double(circleCommand(1));
                height = str2double(circleCommand(1));
                centerX = str2double(circleCommand(2));
                centerY = str2double(circleCommand(3));
                % joint handle
                for i = 1 : 6
                    [r,joint(i)]=sim.simxGetObjectHandle(clientID, strcat('UR10_joint', int2str(i)) , sim.simx_opmode_blocking);
                end
                ...
                %Tinh dong hoc nguoc
                for t = 0:dt:t_max
                    [Xd, dXd] = circleTracjectory(t, height, centerX, centerY, 0.5904, 0.35862); % Vi tri va van toc diem E cho truoc theo thoi gian t
                    ...
                    for k = 1:1:10000
                        ...
                        Xq = [xE; yE; zE];
                        [Xd, dXd] = circleTracjectory(t, height, centerX, centerY, 0.5904, 0.35862);% Goi lai quy dao mong muon
                        Xm = [Xd(1); Xd(2); Xd(3)];
